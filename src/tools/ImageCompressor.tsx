...
export default function ImageCompressor({ sharedState, updateSharedImage }: ImageCompressorProps) {
  ...
  const setLocalImage = (url: string, size?: number, type?: string) => {
    setImage(url);
    if (updateSharedImage) updateSharedImage(url, false);
    if (size) setOriginalSize(formatSize(size));
  ...
  const handleImageLoad = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setLocalImage(url, file.size, file.type);
  };

  const loadFromHistory = async (url: string | null) => {
    if (url) {
      setImage(url);
      ...
      try {
        ...
        setOriginalSize(formatSize(blob.size));
        if (blob.type.startsWith('image/')) {
          setFileType(blob.type);
        }
      } catch (e) {
        setOriginalSize("Reference Image");
      }
    }
  };
      ...
      {!image ? (
        <div className="flex flex-col gap-4">
          <label className="flex flex-col items-center justify-center w-full h-64 border-2 border-dashed rounded-xl cursor-pointer bg-gray-50 hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 transition-colors text-center p-6">
            ...
            <input type="file" accept="image/jpeg, image/png, image/webp" className="hidden" onChange={handleImageLoad} />
          </label>
        ...
        <div className="space-y-6">
          <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
            <p className="text-sm font-medium">Original Size: <span className="text-gray-500">{originalSize}</span></p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full"
            />
            <p className="text-xs text-gray-500 mt-2">Lower quality means smaller file size but more visual degradation.</p>
          </div>
